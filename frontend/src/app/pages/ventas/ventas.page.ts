...
export class VentasPage implements OnInit {
  ...
  readonly stockDisponibleDetalle = computed(() => {
    const idProducto = Number(this.detalleForm.controls.id_producto.value || 0);
  ...
  readonly precioActualDetalle = computed(() => {
    const idProducto = Number(this.detalleForm.controls.id_producto.value || 0);
  ...
  agregarDetalleVenta(): void {
    ...
    const stockDisponible = this.getStockDisponible(
      idProducto,
      Number(this.ventaForm.controls.id_sucursal.value || 0)
    );
    const cantidadActualDraft = this.draftDetalles()
      .filter((item) => item.id_producto === idProducto)
    ...
    if (stockDisponible !== null && cantidadActualDraft + cantidad > stockDisponible) {
  ...
  registrarVenta(): void {
    ...
    if (!this.draftDetalles().length) {
      this.errorMessage.set('Agrega al menos un producto a la venta.');
      return;
    }
    ...
    this.ventasApi.venta
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMessage.set(this.resolveApiError(error, 'No se pudo registrar la venta.'));
        }
      });
  }
  ...
  guardarCliente(): void {
    ...
    request
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMessage.set(this.resolveApiError(error, 'No se pudo guardar el cliente.'));
        }
      });
  }
  ...
  toggleEstadoCliente(cliente: ClienteDto): void {
    ...
    this.ventasApi.clientes
      ...
      .subscribe({
        next: () => {
          this.successMessage.set(
            nextActivo ? 'Cliente activado correctamente.' : 'Cliente desactivado correctamente.'
          );
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMessage.set(this.resolveApiError(error, 'No se pudo actualizar el cliente.'));
        }
      });
  }
  ...
  toggleEstadoListaPrecio(lista: ListaPrecioDto): void {
  ...
  toggleEstadoPrecioProducto(precio: PrecioProductoDto): void {
  ...
  private getPrecioProductoActivo(
    ...
    return this.preciosProducto().find(
      (item) =>
        ...
        item.id_lista_precio === idListaPrecio &&
        item.activo
    );
  }
